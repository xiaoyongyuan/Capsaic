...
import {Form, Input, Button, Select,Empty,Breadcrumb} from "antd";
        ...
        return(
            <div className="rolltack">
                <Breadcrumb className="crumbs">
                    ...
                    <Breadcrumb.Item>
                        <span className="crumbs-item">点名任务</span>
                    </Breadcrumb.Item>
                </Breadcrumb>
                <div className="task-form">
                    ...
                    <Button className="task-formBtn"><Link to="/main/addRollcallTask">新增</Link></Button>
                </div>
                           ...
                           [this.state.taskList.map((v,i)=>(
                               <div className="task-border" key={i}>
                                   <div className="task-img">
                                       <a href={"#/main/rollcallDetail?rollcalldetailcode="+v.rollcallcode+"&devid="+v.devid}><canvas id={"canvas"+(i+1)} width="260px" height="180px" style={{backgroundImage:'url('+this.params.imgUrl[i]+')',backgroundSize:"100% 100%"}} /></a>
                                       <div className={v.scan?"scangif":"scanno"} alt="" />
                                   </div>
                                   <div className="taskBottm">
                                       <div>{v.resultdate}</div>
                                       <div className="taskStatus"><span>{v.cameraname+"-"+v.cname}</span><span style={{color:v.rstatus ===1?"#828DA6":"#FF606D"}}>{v.rstatus ===1?"正常":"报警"}</span></div>
                                   </div>
                                   <div className="rollcallBtn" style={{display:v.scan?"none":"block"}} onClick={()=>this.hanleRollCall(v,i)}>点名</div>
                               </div>
