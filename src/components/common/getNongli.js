...
export default function showDate() {
    ...
    var b = new Array("零", "一", "二", "三", "四", "五", "六", "七", "八", "九");
    ...
    month = date.substring(4, 6) < 10 ? b[date.substring(4, 5)] + b[date.substring(5, 6)] : date.substring(4, 6) == 10 ?
        '零一十' : b[date.substring(4, 5)] + '十' + b[date.substring(5, 6)];
    date = date.substring(6, 8) < 10 ? b[date.substring(6, 7)] + b[date.substring(7, 8)] : date.substring(6, 8) == 10 ||
    date.substring(6, 8) == 20 || date.substring(6, 8) == 30 ? '零' + b[date.substring(6, 7)] + '十' : b[date.substring(6,7)] + '十' + b[date.substring(7, 8)];
    return year + '年' + month + '月' + date + '日 \t农历'+GetLunarDay(yy, mm, dd);
};