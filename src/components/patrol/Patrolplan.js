...
import {Row, Col, Button, Icon, Card, Modal, Form, Input, TimePicker, Checkbox, message,Breadcrumb} from 'antd';
        ...
        return (
            <div className="patrolPlan">
                <Breadcrumb className="crumbs">
                    ...
                    <Breadcrumb.Item>
                        <span className="crumbs-item">巡更计划</span>
                    </Breadcrumb.Item>
                </Breadcrumb>
                <div className="card-planBtn">
                    <Button className="planBtnAdd" onClick={() => this.hanleAddEdit("1")}>添加</Button>
                ...
                <div className="gutter-example">
                        <Row>
                            {
                                this.state.patrolList.map((v, i) => {
                                    return (
                                        <Col key={i} xxl={7} xl={12} className="gutter-row plan-card">
                                            <div className="gutter-box plan-context">
                                                <div className="plan-circle">
                                                    <span className={"plan-title " + this.bgcolor(i)}>{v.pteam.substring(0, 2)}</span>
                                                </div>
                                                <div className="plan-time">
                                                    <div className="planTime-nike">{v.pbdate}:00--{v.pedate}:00</div>
                                                    <div className="planTime-nike" title={this.cameralist(v.cameralist)}>{this.cameralist(v.cameralist)}</div>
                                                    <div>
                                                        <span className="plan-icon plan-edit" onClick={() => this.hanleAddEdit("2", v,i)}>
                                                            <Icon type="form"/>
                                                            <span className="planIcon">编辑</span>
                                                        </span>
                                                        <span className="plan-icon plan-delete" onClick={() => this.hanlePlanDel(v, i)}>
                                                            <Icon type="delete"/>
                                                            <span className="planIcon">删除</span>
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div className="planTips">提示：最多可补充6个巡更！</div>
                </div>
                <Modal
                    title={this.state.operType === "1" ? "添加" : "编辑"}
